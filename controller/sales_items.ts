...
export const addsalesItem = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const result = await pool.query('INSERT INTO Sales_item (sales_ledger_id, product_id, qty, GST, total, discount, subtotal) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [salesLedgerId, productId, qty, GST, total, discount, subtotal]);
...
export const updatesalesitem = async (req: Request, res: Response) => {
    ...
    const { salesLedgerId, productId, qty, GST, total, discount, subtotal } = req.body;
    try {
        const result = await pool.query('update Sales_item set sales_ledger_id = $1, product_id = $2, qty = $3, GST = $4, total = $5, discount = $6, subtotal = $7 where id = $8  returning *',
            [salesLedgerId, productId, qty, GST, total, discount, subtotal, id])
